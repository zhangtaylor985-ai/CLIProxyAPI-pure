...
func (e *CodexWebsocketsExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	body, wsHeaders := applyCodexPromptCacheHeaders(auth, wsURL, from, req, body)
	...
	executionSessionID := codexWebsocketSessionKey(executionSessionIDFromOptions(opts), auth, wsURL)
...
func applyCodexPromptCacheHeaders(auth *cliproxyauth.Auth, endpoint string, from sdktranslator.Format, req cliproxyexecutor.Request, rawJSON []byte) ([]byte, http.Header) {
	headers := http.Header{}
	if len(rawJSON) == 0 {
	...
	isolationKey := codexAuthIsolationKey(auth, endpoint)
...
func codexWebsocketSessionKey(executionSessionID string, auth *cliproxyauth.Auth, endpoint string) string {
	executionSessionID = strings.TrimSpace(executionSessionID)
	if executionSessionID == "" {
		return ""
	}
	return executionSessionID + ":" + codexAuthIsolationKey(auth, endpoint)
}
