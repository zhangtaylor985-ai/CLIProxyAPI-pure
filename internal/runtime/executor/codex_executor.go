...
func (e *CodexExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	for _, line := range lines {
		...
		if detail, ok := helps.ParseCodexUsage(eventData); ok {
			...
			observeCodexPromptCacheUsage(auth, from, req.Model, req.Payload, detail.InputTokens, detail.CachedTokens)
		}
...
func (e *CodexExecutor) executeCompact(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	observeCodexPromptCacheUsage(auth, from, req.Model, req.Payload, compactUsage.InputTokens, compactUsage.CachedTokens)
...
func (e *CodexExecutor) cacheHelper(ctx context.Context, auth *cliproxyauth.Auth, from sdktranslator.Format, url string, req cliproxyexecutor.Request, rawJSON []byte) (*http.Request, error) {
	...
	isolationKey := codexAuthIsolationKey(auth, url)
	if from == "claude" || from == "openai" {
		if promptCacheKey := strings.TrimSpace(gjson.GetBytes(req.Payload, "prompt_cache_key").String()); promptCacheKey != "" {
			cache.ID = codexNamespacedPromptCacheKey(isolationKey, promptCacheKey)
		} else if scope := codexPromptCacheScope(auth, from, req.Model, req.Payload); scope != "" {
			cache = helps.GetOrCreateCodexRollingCache(scope)
		} else if from == "openai" {
			if apiKey := strings.TrimSpace(helps.APIKeyFromContext(ctx)); apiKey != "" {
				cache.ID = codexNamespacedPromptCacheKey(isolationKey, apiKey)
			}
		}
	} else if from == "openai-response" {
		promptCacheKey := gjson.GetBytes(req.Payload, "prompt_cache_key")
		if promptCacheKey.Exists() {
			cache.ID = codexNamespacedPromptCacheKey(isolationKey, promptCacheKey.String())
		} else if scope := codexPromptCacheScope(auth, from, req.Model, req.Payload); scope != "" {
			cache = helps.GetOrCreateCodexRollingCache(scope)
		}
	}
...
func codexPromptCacheScope(auth *cliproxyauth.Auth, from sdktranslator.Format, model string, payload []byte) string {
	identityKind := "metadata_user_id"
	identity := strings.TrimSpace(gjson.GetBytes(payload, "metadata.user_id").String())
	if promptCacheKey := strings.TrimSpace(gjson.GetBytes(payload, "prompt_cache_key").String()); promptCacheKey != "" {
		identityKind = "prompt_cache_key"
		identity = promptCacheKey
	}
	if identity == "" {
		return ""
	}
	source := strings.TrimSpace(from.String())
	if source == "" {
		source = "unknown"
	}
...
func observeCodexPromptCacheUsage(auth *cliproxyauth.Auth, from sdktranslator.Format, model string, payload []byte, inputTokens, cachedTokens int64) {
	helps.ObserveCodexRollingCacheUsage(codexPromptCacheScope(auth, from, model, payload), inputTokens, cachedTokens)
}

func codexAuthIsolationKey(auth *cliproxyauth.Auth, endpoint string) string {
	parts := []string{strings.TrimSpace(endpoint)}
	if auth != nil {
		parts = append(parts,
...
func codexNamespacedPromptCacheKey(isolationKey, rawKey string) string {
	rawKey = strings.TrimSpace(rawKey)
	if rawKey == "" {
		return ""
	}
	return uuid.NewSHA1(uuid.NameSpaceOID, []byte("cli-proxy-api:codex:prompt-cache:"+isolationKey+":"+rawKey)).String()
}
