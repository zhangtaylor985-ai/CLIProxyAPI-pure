...
func TestApplyCodexPromptCacheHeadersSetsLowercaseSessionAndLegacyConversation(t *testing.T) {
	...
	endpoint := "wss://example.com/responses"

	_, headers := applyCodexPromptCacheHeaders(auth, endpoint, "openai-response", req, []byte(`{"model":"gpt-5-codex"}`))

	expected := codexNamespacedPromptCacheKey(codexAuthIsolationKey(auth, endpoint), "cache-1")
	if got := headerValueCaseInsensitive(headers, "session_id"); got != expected {
		t.Fatalf("session_id = %s, want %s", got, expected)
	}
	if _, ok := headers["session_id"]; !ok {
		t.Fatalf("expected lowercase session_id key, got %#v", headers)
	}
	if got := headers.Get("Conversation_id"); got != expected {
		t.Fatalf("Conversation_id = %s, want %s", got, expected)
	}
}

func TestApplyCodexPromptCacheHeadersRollsClaudePromptCache(t *testing.T) {
	req := cliproxyexecutor.Request{Model: "gpt-5.4(high)", Payload: []byte(`{"metadata":{"user_id":"ws-rolling-cache-user"}}`)}
	auth := &cliproxyauth.Auth{ID: "auth-ws-rolling", Provider: "codex", ProxyURL: "socks5://proxy.example:443"}
	endpoint := "wss://example.com/responses"
	body := []byte(`{"model":"gpt-5.4","stream":true}`)

	body0, headers0 := applyCodexPromptCacheHeaders(auth, endpoint, "claude", req, body)
	key0 := gjson.GetBytes(body0, "prompt_cache_key").String()
	if key0 == "" || headerValueCaseInsensitive(headers0, "session_id") != key0 || headers0.Get("Conversation_id") != key0 {
		t.Fatalf("initial websocket cache key/header mismatch: key=%q headers=%v", key0, headers0)
	}
	...
	if key1 == "" || key1 == key0 {
		t.Fatalf("expected websocket rolling cache key to advance, key0=%q key1=%q", key0, key1)
	}
	if headerValueCaseInsensitive(headers1, "session_id") != key1 || headers1.Get("Conversation_id") != key1 {
		t.Fatalf("rolled websocket cache key/header mismatch: key=%q headers=%v", key1, headers1)
	}
}

func TestApplyCodexPromptCacheHeadersRollsOpenAIMetadataPromptCache(t *testing.T) {
	req := cliproxyexecutor.Request{Model: "gpt-5.4(medium)", Payload: []byte(`{"metadata":{"user_id":"ws-openai-rolling-user"}}`)}
	auth := &cliproxyauth.Auth{ID: "auth-ws-openai-rolling", Provider: "codex", ProxyURL: "socks5://proxy.example:443"}
	endpoint := "wss://example.com/responses"
	body := []byte(`{"model":"gpt-5.4","stream":true}`)

	body0, headers0 := applyCodexPromptCacheHeaders(auth, endpoint, "openai", req, body)
	key0 := gjson.GetBytes(body0, "prompt_cache_key").String()
	if key0 == "" || headerValueCaseInsensitive(headers0, "session_id") != key0 || headers0.Get("Conversation_id") != key0 {
		t.Fatalf("initial OpenAI websocket cache key/header mismatch: key=%q headers=%v", key0, headers0)
	}
	...
	key1 := gjson.GetBytes(body1, "prompt_cache_key").String()
	if key1 == "" || key1 == key0 {
		t.Fatalf("expected OpenAI websocket rolling cache key to advance, key0=%q key1=%q", key0, key1)
	}
	if headerValueCaseInsensitive(headers1, "session_id") != key1 || headers1.Get("Conversation_id") != key1 {
		t.Fatalf("rolled OpenAI websocket cache key/header mismatch: key=%q headers=%v", key1, headers1)
	}
}

func TestApplyCodexPromptCacheHeadersUsesOpenAIForwardedPromptCacheKey(t *testing.T) {
	reqA := cliproxyexecutor.Request{Model: "gpt-5.4(medium)", Payload: []byte(`{"prompt_cache_key":"ws-session-a","metadata":{"user_id":"same-user"}}`)}
	reqB := cliproxyexecutor.Request{Model: "gpt-5.4(medium)", Payload: []byte(`{"prompt_cache_key":"ws-session-b","metadata":{"user_id":"same-user"}}`)}
	auth := &cliproxyauth.Auth{ID: "auth-ws-openai-forwarded", Provider: "codex", ProxyURL: "socks5://proxy.example:443"}
	endpoint := "wss://example.com/responses"
	body := []byte(`{"model":"gpt-5.4","stream":true}`)

	bodyA, headersA := applyCodexPromptCacheHeaders(auth, endpoint, "openai", reqA, body)
	bodyB, headersB := applyCodexPromptCacheHeaders(auth, endpoint, "openai", reqB, body)
	keyA := gjson.GetBytes(bodyA, "prompt_cache_key").String()
	keyB := gjson.GetBytes(bodyB, "prompt_cache_key").String()
	if keyA == "" || keyB == "" {
		t.Fatalf("expected forwarded OpenAI websocket cache keys, got A=%q B=%q", keyA, keyB)
	}
	if keyA == keyB {
		t.Fatalf("different forwarded OpenAI websocket cache keys shared generated key %q", keyA)
	}
	if headerValueCaseInsensitive(headersA, "session_id") != keyA || headersA.Get("Conversation_id") != keyA {
		t.Fatalf("forwarded websocket cache key/header mismatch A: key=%q headers=%v", keyA, headersA)
	}
	if headerValueCaseInsensitive(headersB, "session_id") != keyB || headersB.Get("Conversation_id") != keyB {
		t.Fatalf("forwarded websocket cache key/header mismatch B: key=%q headers=%v", keyB, headersB)
	}

	observeCodexPromptCacheUsage(auth, sdktranslator.FromString("openai"), reqA.Model, reqA.Payload, 36387, 18432)
	bodyA2, headersA2 := applyCodexPromptCacheHeaders(auth, endpoint, "openai", reqA, body)
	keyA2 := gjson.GetBytes(bodyA2, "prompt_cache_key").String()
	if keyA2 != keyA {
		t.Fatalf("forwarded websocket cache key should remain stable after cached growth: got %q want %q", keyA2, keyA)
	}
	if headerValueCaseInsensitive(headersA2, "session_id") != keyA2 || headersA2.Get("Conversation_id") != keyA2 {
		t.Fatalf("forwarded websocket cache key/header mismatch after cached growth: key=%q headers=%v", keyA2, headersA2)
	}
}

func TestCodexWebsocketSessionKeySeparatesSameExecutionSessionByAuth(t *testing.T) {
	...
	keyA := codexWebsocketSessionKey("downstream-session", authA, endpoint)
	keyAAgain := codexWebsocketSessionKey("downstream-session", authA, endpoint)
	keyB := codexWebsocketSessionKey("downstream-session", authB, endpoint)
	if keyA == "" || keyB == "" {
		t.Fatalf("session keys should be set, got A=%q B=%q", keyA, keyB)
	}
	if keyA != keyAAgain {
		t.Fatalf("same auth/session should produce stable key: %q != %q", keyA, keyAAgain)
	}
	if keyA == keyB {
		t.Fatalf("different auths must not share websocket session key %q", keyA)
	}

	sessA := executor.getOrCreateSession(keyA)
	sessB := executor.getOrCreateSession(keyB)
	if sessA == nil || sessB == nil {
		t.Fatalf("sessions should be created")
	}
	if sessA == sessB {
		t.Fatalf("same downstream execution session with different auths reused one upstream websocket session")
	}
}
